...
namespace nm
{
    ImageSaver::ImageSaver(const ChunkSource& chunkSource):
        chunkSource(chunkSource), face(library, "./berry.pcf.gz")
    {
        ft::set_face_size(face, 8);
    };

    void ImageSaver::write_block(uint32_t charcode, FT_Vector position, FT_Vector offset,
    ...
    void ImageSaver::save(const std::string& filename)
    {
        ...
        this->image =
            png::image<png::rgb_pixel>((end_x - begin_x) * NM_CHUNK_SIZE * glyph_size.x * 3,
